...
/** Google Maps deep link for the venue's plus code — no API key needed. */
    ...
    <>
      ...
      <section id="tickets-plans" style={{ position: 'relative', background: '#fff', color: '#0E1667', padding: '76px 40px' }}>
        ...
        <div id="tickets-plans-grid" style={{ maxWidth: '1140px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '24px', alignItems: 'stretch' }}>
          {TICKET_PLANS.map((plan, i) => (
            <div key={plan.label} data-reveal data-reveal-d={String(i * 60)} style={{ ...cardStyle, borderTop: `4px solid ${plan.accent}`, display: 'flex', flexDirection: 'column' }}>
      ...
      <section id="tickets-highlights" style={{ position: 'relative', background: '#F4F1E8', color: '#0E1667', padding: '76px 40px 84px' }}>
        <div data-reveal style={{ maxWidth: '1140px', margin: '0 auto 32px', textAlign: 'center' }}>
          ...
          <h2 style={{ margin: '0 0 12px', fontWeight: 500, fontSize: 'clamp(26px,3.6vw,40px)', letterSpacing: '-1px' }}>Past Event Highlights</h2>
